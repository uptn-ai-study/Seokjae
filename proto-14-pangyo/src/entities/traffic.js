...
export class Traffic {
  ...
  update(dt, cam, player, game) {
    ...
    for (let i = this.cars.length - 1; i >= 0; i--) {
      ...
      this._driveAI(c, dt, player, game);
  ...
  _driveAI(c, dt, player, game) {
    ...
    for (const o of this.cars) {
      if (o === c) continue;
      if (Math.abs(o.v.x - px) < 34 && Math.abs(o.v.y - py) < 34) { blockedAhead = true; break; }
    }
    if (!blockedAhead) {
      const pv = game.playerVehicle;
      const t = pv || player;
      if (Math.abs(t.x - px) < 36 && Math.abs(t.y - py) < 36) blockedAhead = true;
    }
    if (!blockedAhead) {
    ...
    if (blockedAhead) {
      c.wait += dt;
      v.ctrl.throttle = c.wait > 3.5 ? 0.4 : -0.6; // 너무 오래 막히면 슬쩍 밀고 나간다
    } else {
      c.wait = 0;
